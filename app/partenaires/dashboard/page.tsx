...
function formatSurface(t: string | null) {
  if (!t) return "—"
  const m: Record<string, string> = { toiture: "Toiture", parking: "Parking", friche: "Friche", terrain: "Terrain" }
...
function GlassCard({
  ...
  return (
    <div
      className={`rounded-2xl border border-white/[0.08] bg-white/[0.03] shadow-[inset_0_1px_0_0_rgba(255,255,255,0.05)] backdrop-blur-xl ${className}`}
    >
      {children}
    </div>
  )
...
/* ═══════════════════════════════════════════════
   Component: FOMO Slot Indicators (B2C)
   ═══════════════════════════════════════════════ */
...
function SlotIndicators({ slots }: { slots: MarketplaceLead["slots"] }) {
  ...
  const items = Array.from({ length: slots.max }, (_, i) => {
    ...
    if (buyer) {
      return buyer.isMe
        ? { label: "Vous", color: "bg-[#FFC300]/20 text-[#FFC300] border-[#FFC300]/30" }
...
/* ═══════════════════════════════════════════════
   Component: Lead Card (Marketplace Teaser)
   ═══════════════════════════════════════════════ */
...
function LeadCard({
  ...
  return (
    <motion.div
      ...
      layout
    >
      <GlassCard className="group relative overflow-hidden transition-all duration-300 hover:border-white/[0.15] hover:shadow-[0_0_40px_rgba(255,195,0,0.06)]">
        {/* Ambient glow on hover */}
        <div className="pointer-events-none absolute -right-12 -top-12 h-32 w-32 rounded-full bg-[#FFC300]/[0.04] opacity-0 blur-[60px] transition-opacity group-hover:opacity-100" />

        <div className="p-5">
          {/* Header: Province + Segment Badge */}
            ...
            <Badge
              className={`shrink-0 border text-[10px] font-bold uppercase tracking-wider ${
                isB2C
                  ...
                  : "border-[#FFC300]/30 bg-[#FFC300]/15 text-[#FFC300]"
              }`}
            >
              {lead.segment}
            </Badge>
          ...
          <div className="mb-4 grid grid-cols-2 gap-3">
            <div className="rounded-xl bg-white/[0.04] px-3 py-2.5">
              <div className="flex items-center gap-1.5">
                <Zap className="h-3 w-3 text-[#FFC300]/70" />
            ...
            <div className="rounded-xl bg-white/[0.04] px-3 py-2.5">
              ...
              <p className="mt-1 text-lg font-bold tabular-nums text-[#FFC300]">
                {lead.estimatedRevenue.toLocaleString("fr-BE")}{" "}
                <span className="text-xs font-normal text-[#FFC300]/60">€/an</span>
              </p>
            </div>
          </div>
          ...
          {isMine ? (
            <div className="rounded-xl border border-[#FFC300]/20 bg-[#FFC300]/[0.06] px-4 py-3 text-center">
              <CheckCircle2 className="mx-auto h-5 w-5 text-[#FFC300]" />
              <p className="mt-1 text-xs font-semibold text-[#FFC300]">Débloqué</p>
            </div>
          ) : soldOut ? (
            ...
            <Button
              ...
              disabled={!!purchasing}
              className="h-11 w-full gap-2 rounded-xl bg-[#FFC300] text-sm font-bold text-[#001D3D] shadow-[0_0_20px_rgba(255,195,0,0.2)] transition-all hover:scale-[1.02] hover:bg-[#e6b000] hover:shadow-[0_0_30px_rgba(255,195,0,0.35)] disabled:opacity-50"
            >
              {isPurchasing ? (
        ...
        <AnimatePresence>
          {revealedLead && (
            ...
            >
              <div className="border-t border-[#FFC300]/20 bg-[#FFC300]/[0.03] px-5 py-4">
                <p className="mb-3 text-[10px] font-bold uppercase tracking-[0.2em] text-[#FFC300]">
...
/* ═══════════════════════════════════════════════
   Component: Purchased Lead Card (full PII)
   ═══════════════════════════════════════════════ */
...
function PurchasedLeadCard({
  ...
  return (
    <motion.div
      ...
      animate="visible"
    >
      <GlassCard className="overflow-hidden border-[#FFC300]/10">
        <div className="flex items-center gap-3 border-b border-white/[0.06] px-5 py-3">
          <Badge className="border-[#FFC300]/30 bg-[#FFC300]/15 text-[10px] font-bold uppercase tracking-wider text-[#FFC300]">
            Débloqué
          </Badge>
        ...
        <div className="grid gap-4 p-5 sm:grid-cols-2">
          <div className="space-y-2.5">
            <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-[#FFC300]/70">
              Contact
            </p>
          ...
          <div className="space-y-2.5">
            ...
            <div className="space-y-1.5 text-sm">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-medium text-[#FFC300]">
...
/* ═══════════════════════════════════════════════
   Component: Credit Counter (animated)
   ═══════════════════════════════════════════════ */
...
function CreditCounter({ value }: { value: number }) {
  return (
    <motion.span
      ...
      className="text-2xl font-bold tabular-nums text-[#FFC300]"
    >
      {value}
    </motion.span>
  )
...
export default function PartnerDashboardPage() {
  ...
  const fetchMyLeads = useCallback(async () => {
    try {
      ...
      if (!res.ok) return
      const data = await res.json()
      const purchasedIds = (data.purchases ?? []).map(
        (p: { leads: RevealedLead }) => p.leads
      )
      setMyLeads(purchasedIds.filter(Boolean))
    } catch {
      /* silent */
    }
  }, [])
  ...
  return (
    <div className="relative min-h-screen bg-[#001D3D]">
      ...
      <div className="pointer-events-none fixed inset-0 z-0">
        <div className="absolute -left-40 top-20 h-[600px] w-[600px] rounded-full bg-[#FFC300]/[0.03] blur-[150px]" />
      ...
      <header className="sticky top-0 z-30 border-b border-white/[0.06] bg-[#001D3D]/80 backdrop-blur-xl">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-4 sm:px-6 lg:px-8">
          <div className="flex items-center gap-3 min-w-0">
            <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-[#FFC300]/10 ring-1 ring-[#FFC300]/20">
          ...
          <div className="flex items-center gap-3">
            {/* Credit Badge */}
            <GlassCard className="flex items-center gap-2.5 px-4 py-2">
            ...
            <Link href="/partenaires/dashboard/credits">
              <Button className="h-10 gap-2 rounded-xl border border-[#FFC300]/20 bg-[#FFC300]/10 px-4 text-sm font-semibold text-[#FFC300] hover:bg-[#FFC300]/20">
      ...
      <main className="relative z-10 mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        <Tabs value={tab} onValueChange={setTab} className="w-full">
          <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-neutral-500">
                Leads solaires qualifiés · Wallonie · Temps réel
              </p>
            </div>
            <div className="flex items-center gap-3">
              <Button
              ...
              <TabsList className="h-10 border border-white/[0.08] bg-white/[0.04]">
                <TabsTrigger
                  value="marketplace"
                  className="px-4 text-xs data-[state=active]:bg-[#FFC300]/15 data-[state=active]:text-[#FFC300]"
                >
                  Marketplace
                </TabsTrigger>
                <TabsTrigger
          ...
          <TabsContent value="marketplace" className="mt-0">
            {leads.length === 0 ? (
              <GlassCard className="px-8 py-16 text-center">
                <Zap className="mx-auto h-8 w-8 text-neutral-600" />
                <p className="mt-3 text-sm text-neutral-500">
                  Aucun lead disponible pour le moment. Revenez bientôt.
                </p>
              </GlassCard>
            ) : (
          ...
          <TabsContent value="purchased" className="mt-0">
            {myLeads.length === 0 ? (
              <GlassCard className="px-8 py-16 text-center">
                <Lock className="mx-auto h-8 w-8 text-neutral-600" />
                <p className="mt-3 text-sm text-neutral-500">
                  Vous n&apos;avez pas encore débloqué de lead.
                </p>
                <Button
                  onClick={() => setTab("marketplace")}
                  className="mt-4 gap-2 rounded-xl bg-[#FFC300]/15 text-sm text-[#FFC300] hover:bg-[#FFC300]/25"
                >
                  Voir la marketplace
